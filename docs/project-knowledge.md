...
**다음 후보(사용자 확인 필요)**: (1) 세 시나리오가 근소하게 못 넘은 만큼, 더 큰 timesteps 예산이나 더 큰 pool로 재시도, (2) 이번에 의도적으로 제외한 learning rate 감쇠를 domain randomization 학습에도 적용, (3) ~~pool 크기·held-out 개수를 늘려 이 결과가 우연이 아닌지 재확인~~ → 6.18절에서 수행.

### 6.18 Domain randomization 개선의 재현성 확인 (learning seed 4개 × held-out 20개)

**상태:** 관찰
**마지막 갱신:** 2026-07-30

6.17절의 결과는 learning seed 1개(11)·held-out 5개짜리 단일 관측이라 "재현되는 개선인지 우연인지" 구분할 수 없었다. 이를 확인하기 위해 `tools/stage14_domain_randomization_check.py`에 `--learning-seed`/`--held-out-scenario-seeds` CLI를 추가해, learning seed 4개(11, 23, 37, 41 — 기존 8-seed 실험과 겹치는 값)를 각각 500,000 timesteps로 학습하고, held-out을 20개(seed 1001~1020, 기존 5개를 포함하는 상위집합)로 확장해 평가했다.

| learning seed | 세 휴리스틱 전부 이김 | random_valid 이김 | held-out 20개 평균 마진(휴리스틱 최저 대비) | 원래 5개(1001~1005) 평균 마진 |
|---|---|---|---|---|
| 11 | 9/20 | 19/20 | -0.14 | -0.27 |
| 23 | 8/20 | 15/20 | -0.70 | 0.00 |
| 37 | 5/20 | 17/20 | -0.94 | -0.52 |
| 41 | 12/20 | 20/20 | +0.69 | +0.22 |
| 평균 | 34/80 (42.5%) | 71/80 (88.75%) | -0.27 | -0.14 |

**핵심 결론: 4개 seed 전부에서 원래 5개 held-out에 대한 평균 마진이 -5.10(6.16절 단일 시나리오 기준선)에서 -0.52~+0.22 사이로 개선됐다.** 즉 6.17절의 개선이 seed 11 하나의 우연이 아니라 재현되는 효과라는 것을 확인했다. 다만 seed마다 "얼마나 잘하는가"의 편차는 여전히 크다(세 휴리스틱 전부 이긴 비율이 25%~60%).

**시나리오별 패턴도 확인했다** — 20개 held-out 중:
- 3개(1003, 1009, 1013)는 4개 seed 전부가 이겼다(항상 이기는 "쉬운" unseen 시나리오).
- 5개(1004, 1006, 1014, 1016, 1018)는 4개 seed 전부가 졌다(항상 지는 "어려운" unseen 시나리오 — 휴리스틱이 이 구조에서는 이미 거의 최적이거나, PPO가 일반화하지 못하는 패턴일 수 있다. 원인은 검토 필요).
- 나머지 12개는 seed에 따라 갈렸다.

**학습 곡선에서 seed별 개별 특성도 관찰했다**: seed 37은 held-out 곡선이 중간에 54.93까지 크게 떨어졌다가 62.64로 겨우 회복했고(시작 62.70과 거의 같은 수준으로 끝남 — 사실상 순 개선 없음), 성적도 4개 중 가장 낮았다(5/20, 평균 마진 -0.94). **seed 37은 6.12절의 원래 단일 시나리오 8-seed 실험에서도 유일하게 "찾은 좋은 지점을 완전히 잃어버린" seed였다** — 이 seed가 시나리오와 무관하게 이 신경망 초기화/탐색 조합 자체에서 불안정한 경향이 있을 가능성을 시사한다(가정, 추가 seed로 재검증 필요). 반대로 seed 41은 곡선이 처음부터 끝까지 큰 변화 없이(63.63→63.44) 이미 높은 수준을 유지했고 성적도 가장 좋았다(12/20).

**부수적으로**: 이 재현성 sweep 도중 컴퓨터가 세 번째로 예기치 않게 재부팅됐다. seed 11은 이미 완료된 상태였고, seed 23은 checkpoint 300,000/500,000에서 중단돼(미완료로 판단해) 처음부터 다시 학습했다. seed 37/41은 재부팅 이후 새로 시작했다 — 완료된 seed 11 덕분에 전체를 처음부터 다시 돌릴 필요는 없었다.

**다음 후보**: (1) 항상 지는 5개 시나리오(1004/1006/1014/1016/1018)의 공통 구조 분석, (2) seed 37 같은 불안정 seed가 domain randomization에서도 반복되는지 더 많은 seed로 확인, (3) learning rate 감쇠를 seed 37류의 불안정성에 타겟해서 적용, (4) 더 큰 pool/budget으로 평균 마진을 양수로 끌어올리는 시도.
