...
2026-07-26 4차 배치는 마지막 통합 검증 항목 "RL과 모든 기준 정책 비교"를 완료해 통합 검증 10/10이 됐다. `tools/stage14_full_scale_direction_check.py`로 full 시나리오에서 Maskable PPO(2 seed, 각 30,000 timesteps)를 4개 baseline 휴리스틱과 CP-SAT(`time_limit_sec=900`) 전부와 처음으로 비교했다. PPO median(98.42)은 Random valid median(97.30)보다는 나았지만 나머지 세 휴리스틱(100.35~101.44)과 CP-SAT 최적해(104.59, `OPTIMAL`)에는 못 미쳤고, 두 seed의 학습 곡선이 서로 다른 양상(한 seed는 뚜렷한 상승 후 약간 하락, 다른 seed는 초반부터 평평)을 보였다. 상세는 `docs/project-knowledge.md` 6.12절. 이번에도 PPO가 모든 기준을 이겨야 한다는 합격선은 두지 않았고, 실제 정량 비교가 처음 이뤄졌다는 사실로 완료 조건("RL 정책이 최소한 Random valid와 정량적으로 비교된다")을 충족했다.

첫 실행 중 컴퓨터가 예기치 않게 재부팅되어(원인 불명, 이번 작업과의 연관 근거 없음 — Windows 이벤트 로그에 BSOD·메모리 고갈 징후 없음) 중단됐고, 처음부터 재실행해 완료했다.

남은 문서 7개 항목은 더 큰 작업 단위라 후속 세션에서 진행한다.
